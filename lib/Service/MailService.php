...
class MailService {
	...
	private function getLangForEmail(string $email): IL10N {
		$users = $this->userManager->getByEmail($email);

		if (!empty($users)) {
			$lang = $this->l10nFactory->getUserLanguage($users[0]);
		} else {
			$lang = $this->l10nFactory->getUserLanguage(null) ?? $this->l10n->getLanguageCode();
		}

		return $this->l10nFactory->get('libresign', $lang);
	}
	...
	public function notifySignDataUpdated(SignRequest $data, string $email, ?string $description = null): void {
		...
		$emailTemplate->setSubject($l->t('LibreSign: Changes into a file for you to sign'));
		$emailTemplate->addHeader();
		$emailTemplate->addHeading($l->t('File to sign'), false);
		...
		$emailTemplate->addBodyText($l->t('Changes have been made in a file that you have to sign. Access the link below:'));
	...
	public function notifyUnsignedUser(SignRequest $data, string $email, ?string $description = null): void {
		...
		$emailTemplate = $this->mailer->createEMailTemplate('settings.TestEmail');
		$emailTemplate->setSubject($l->t('LibreSign: There is a file for you to sign'));
		$emailTemplate->addHeader();
		$emailTemplate->addHeading($l->t('File to sign'), false);
		...
		$emailTemplate->addBodyText($l->t('There is a document for you to sign. Access the link below:'));
		$link = $this->urlGenerator->linkToRouteAbsolute('libresign.page.sign', ['uuid' => $data->getUuid()]);
		$file = $this->getFileById($data->getFileId());
		$emailTemplate->addBodyButton(
			$l->t('Sign »%s«', [$file->getName()]),
	...
	public function notifySignedUser(SignRequest $signRequest, string $email, File $libreSignFile, string $displayName): void {
		...
		$emailTemplate->setSubject($l->t('LibreSign: A file has been signed'));
		$emailTemplate->addHeader();
		$emailTemplate->addHeading($l->t('File signed'), false);
		...
		$emailTemplate->addBodyText($l->t('%s signed the document. You can access it using the link below:', [$signRequest->getDisplayName()]));
		...
		$emailTemplate->addBodyButton(
			...
			$l->t('View signed file »%s«', [$file->getName()]),
			$link
		);
		$message = $this->mailer->createMessage();
	...
	public function notifyCanceledRequest(SignRequest $signRequest, string $email, File $libreSignFile): void {
		...
		$emailTemplate->setSubject($l->t('LibreSign: A signature request has been canceled'));
		$emailTemplate->addHeader();
		$emailTemplate->addHeading($l->t('Signature request canceled'), false);
		...
		$emailTemplate->addBodyText($l->t('The request for you to sign "%s" has been canceled.', [$libreSignFile->getName()]));
		$message = $this->mailer->createMessage();
	...
	public function sendCodeToSign(string $email, string $name, string $code): void {
		...
		$emailTemplate = $this->mailer->createEMailTemplate('settings.TestEmail');
		$emailTemplate->setSubject($l->t('LibreSign: Code to sign file'));
		$emailTemplate->addHeader();
		$emailTemplate->addBodyText($l->t('Use this code to sign the document:'));
